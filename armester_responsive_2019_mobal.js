...
let buttons = [
    ...
    {title: 'Egyéni arckép', content: '<p><b><i>Egyéni arcképe beállításra került. – jim bcs – 2019.xx.xx.</i></b></p>'},
...
(function() {
    let msgList = document.getElementsByClassName('msg-list')[1];
    if (msgList != undefined) {
        let list = msgList.getElementsByClassName('list-unstyled')[0];
        let observer = new MutationObserver(mutationList => {
            let m = mutationList.filter(e => Array.from(e.addedNodes).filter(f => f.classList != undefined && f.classList.contains('msg-editor')).length > 0);
            if (m.length > 0) {
